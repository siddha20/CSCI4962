...
\begin{description}
    ...
    \bgc 
    $\dfrac{\partial u}{\partial t} = \kappa \dfrac{\partial^2 u}{\partial t^2}$.
    \enc
    ...
    \bgc
    \scalebox{.8}{
    $\alpha\Biggl[\dsum{i=0}{n}\Biggl(\dfrac{\partial \hat{u}_i}{\partial x_i} - \dfrac{\partial^2 \hat{u}_i}{\partial t_i^2}\Biggr)^2\Biggr]^{1/2} 
    ...
    \enc
    After finding the optimal hyperparameters and training the neural network,
    ...
    \end{minipage}
    As you can see, the neural network approximates the solution well. The two
    plots on top (the exact and approximate solution) represent the plots of
    $u(x, t)$, where the lighter lines have a $t$ closer to 0. For the heatmaps
    on the bottom, a darker shade is cooler, and the time increases in the
    downward direction of the $y$-axis.
